const routes = {
  glp1: {
    title: 'Снижать вес и сохранять силы по пути',
    kicker: 'Снижение веса · GLP‑1',
    deck: 'Препараты на основе кишечного гормона GLP‑1 помогают насыщаться меньшей порцией. Врач определит, подходит ли такое лечение, а питание, переносимость, талия и сила останутся в одной программе.',
    image: 'assets/hero-glp1-v2.webp',
    imageMobile: 'assets/hero-glp1-mobile-v2.webp',
    ...
    facts: [
      ['Ясно, подходит ли препарат', 'Врач сопоставит цель, состояние, лекарства и ограничения и выберет подходящий вариант снижения веса.', '28%'],
      ['Меньше объём, больше пользы', 'Рацион перестраивается под меньший аппетит так, чтобы в порции оставалось место белку, жидкости и нужной энергии.', '46%'],
      ...
      ['Программа помещается в жизнь', 'Специалист по сопровождению помогает соединить лечение, еду и движение с рабочей неделей, поездками и привычным ритмом.', '82%'],
    ...
    spire: 'Назначение — первая точка\nДальше начинается путь к результату',
    nextKicker: 'Практический разбор',
    nextTitle: 'Как собрать рацион, когда препарат уменьшил аппетит',
    nextCopy: 'Две крайности, белок без магической цифры, сила, талия и честный план на случай отмены препарата.',
    nextImage: 'assets/article-protein.webp',
    nextHref: 'article-protein-glp1.html',
  ...
  longevity: {
    ...
    nextTitle: 'Фисетин и клетки, которые перестали делиться',
    nextCopy: 'Почему эти клетки исследуют, как гипотеза дошла до клиник Mayo и что первые данные у людей позволяют сказать сегодня.',
    nextImage: 'assets/article-fisetin-v2.webp',
    nextHref: 'article-senolytics-fisetin.html',
...
function renderRoute() {
  ...
  heroImage.alt = '';
  const heroSource = $('[data-route-source]');
  if (heroSource) heroSource.srcset = route.imageMobile || route.image;
  const nextImage = $('[data-next-image]');
  nextImage.src = route.nextImage;
  nextImage.alt = '';
  const nextLink = $('[data-next-link]');
  if (nextLink) nextLink.href = route.nextHref || '#route-contact';
